...
# The local products area directory name has the version and qualifier of @$PROJECT@ that you
...
function create_master_cmake_file()
{
  ...
  # --- Start of HERE document for srcs/CMakeLists.txt ---

  # --- Comments below pertain to that file ---
  cat > srcs/CMakeLists.txt << EOF
# Master build script for CMake (automatically generated by "mrb newDev")

# Require minimum cmake version
CMAKE_MINIMUM_REQUIRED (VERSION 2.8)

# Setup the local products area
SET (CMAKE_INSTALL_PREFIX \$ENV{CMAKE_INSTALL_PREFIX} )

# Add top level include
include_directories ( \${PROJECT_SOURCE_DIR} )

# On install, delete the "source_for_local.sh" file and set up a new one
INSTALL (CODE "execute_process(COMMAND rm -f \${CMAKE_INSTALL_PREFIX}/source_for_local.sh ERROR_QUIET)" )
INSTALL (CODE "file( WRITE \${CMAKE_INSTALL_PREFIX}/source_for_local.sh \"# Source this file to set up products in your local products area\" ) ")

# add_subdirectory commands will be added below by @mrb getRedmineGit@

# Add @add_subdirectory( directory )@ by hand if you added by hand a directory you want to build

# Remove the appropriate line below if you delete a product from your srcs area
EOF
# --- End of HERE document for srcs/CMakeLists.txt ---
...
# Make sure the @$PROJECT@ product is setup
if [ -z $PROJECT ]
then
    echo "ERROR: PROJECT is not defined."
    echo "       Please set \$PROJECT to the desired project (e.g., larsoft, uboone, etc.)"
    exit 2
fi
PROJECTUC=`echo ${PROJECT} | tr '[:lower:]' '[:upper:]'`
prjvername="${PROJECTUC}_VERSION"
prjver="${!prjvername}"
prjdirname="${PROJECTUC}_DIR"
prjdir="${!prjdirname}"
if [ -z ${prjver} ]
then
    echo "ERROR: $PROJECT product is not setup. You must setup $PROJECT to the desired version"
    echo "       Available versions of $PROJECT:"
    ups list -aK+ $PROJECT
    exit 2
fi
echo "building development area for $PROJECT ${prjver}"
...
if [ ! $justLP ];  then
  ...
  # @fileTemplates@). See &l=fileTemplates/setup_for_development_top& for the template
  cp $thisDirA/../fileTemplates/setup_for_development_top srcs/setup_for_development
  ...
  if ups flavor -1 | grep -q 'Darwin'; then
    cp $thisDirA/../fileTemplates/xcodeBuild.sh srcs/xcodeBuild.sh
...
if [ $doLP ]; then
    ...
    dirVerQual="_${PROJECT}_${prjver}_${prjqual}"

    # Construct the call for the setup
    setupLine="setup $PROJECT ${prjver} -q \"${project_qual}\""
